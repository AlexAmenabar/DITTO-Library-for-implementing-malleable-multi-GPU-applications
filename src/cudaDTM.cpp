...
void initializeStreams(jobResources_t *jobResources){
    ...
    for(size_t i = 0; i<nGPUs; i++){
        ...
        // create stream for the GPU (if it does not exist yet)
        if(!getCudaStreams()[gpu]){
            
            cudaStreamCreate(&(getCudaStreams()[gpu]));
        }
    }
}
...
void destroyStreams(jobResources_t *jobResources){
    ...
    for(size_t i = 0; i<nGPUs; i++){
        ...
        cudaStreamDestroy(getCudaStreams()[gpu]);

        // set to 0
        getCudaStreams()[gpu] = 0;
    } 
}
...
void initializeNCCLComm(jobResources_t *jobResources){
    ...
    appData->ncclComms = (ncclComm_t*)malloc(nGPUs * sizeof(ncclComm_t));
    
    ncclCommInitAll(appData->ncclComms, nGPUs, devs.data());
}
...
void cpyDataCPU2GPUtwoSteps(DTI_t *DTI){
    ...
    for(i = 0; i<nGPUs; i++){
        ...
        if(commType.transmissionType == asyncComm) // async
            err = cudaMemcpyAsync(DTI->gpuData[i], (char*)cData[i], DTI->nPerGPU[i] * size, cudaMemcpyHostToDevice, cudaStreams[gpuId]);
        else // sync
    ...
    if(commType.transmissionType == asyncComm){
        ...
        for(i = 0; i<nGPUs; i++){
            ...
            cudaStreamSynchronize(cudaStreams[gpuIds[i]]);
        ...
        if(commType.transferSteps == oneStepComm){
            ...
            for(j = 0; j<nPartitionsGPU; j++){
                ...
                if(nPartitionGPU > 0){
                    ...
                    // async communication
                    if(commType.transmissionType == asyncComm)
                        err = cudaMemcpyAsync(dst, src, nBytes, cudaMemcpyHostToDevice, cudaStreams[gpuIds[i]]);
            ...
            if(commType.transmissionType == asyncComm)
                err = cudaMemcpy2DAsync(dst, dpitch, src, spitch, w, h, cudaMemcpyHostToDevice, cudaStreams[gpuIds[i]]);
...
void cpyDataGPU2CPUtwoSteps(DTI_t *DTI){
    ...
    for(i = 0; i<nGPUs; i++){
        ...
        if(commType.transmissionType == asyncComm)
            err = cudaMemcpyAsync((char*)cData[i], DTI->gpuData[i], DTI->nPerGPU[i] * size, cudaMemcpyDeviceToHost, cudaStreams[gpuIds[i]]);
        ...
        if(commType.transferSteps == oneStepComm){
            ...
            for(j = 0; j<nPartitionsGPU; j++){
                ...
                if(nPartitionGPU>0){
                    ...
                    if(commType.transmissionType == asyncComm)
                        err = cudaMemcpyAsync(dst, src, nBytes, cudaMemcpyDeviceToHost, cudaStreams[gpuIds[i]]);
            ...
            if(commType.transmissionType == asyncComm)
                err = cudaMemcpy2DAsync(dst, dpitch, src, spitch, w, h, cudaMemcpyDeviceToHost, cudaStreams[gpuIds[i]]);
...
void reconfExpand(DTI_t *DTI){
    ...
    // allocate memory for intermediate CPU buffers
    float **hostBuffer = (float**)calloc(nReconfGPUs, sizeof(float*));
    ...
    for(i = 0; i<nGPUs; i++){
        ...
        for(j = 0; j<M; j++){
            ...
            if(canAccess){
                    ...
                    for(size_t nP = 0; nP < nPartitions; nP++){
                        ...
                        if (canAccess){
                            ...
                            if(commType.transmissionType == asyncComm){
                                ...
                                err = cudaMemcpyPeerAsync(dst, dstDev, src, srcDev, nBytes, cudaStreams[idGPUs[i]]);
                            }
                            else{
                            ...
                            if(commType.transmissionType == asyncComm){
                                ...
                                err = cudaMemcpyAsync(dst, src, nBytes, cudaMemcpyDeviceToDevice, cudaStreams[idGPUs[i]]);
                ...
                // strided communication function
                    ...
                    */
                    ...
                    if(commType.transmissionType == asyncComm){
                        err = cudaMemcpy2DAsync(dst, dpitch, src, spitch, w, h, cudaMemcpyDeviceToDevice, cudaStreams[idGPUs[i]]);
                    }
            ...
            // move data firt to the CPU, and then to the GPU
            else{

                // allocate memory for the host buffer
                hostBuffer[dstDevIndex] = (float*)malloc(DTI->nPerGPU[dstDevIndex] * sizeof(float));
                ...
                */
                ...
                // move data
                cudaSetDevice(srcDev);
                cudaMemcpy2DAsync(hostBuffer[dstDevIndex], dpitch, src, spitch, w, h, cudaMemcpyDeviceToHost, cudaStreams[srcDev]);
                err = cudaStreamSynchronize(cudaStreams[srcDev]);
                
                cudaSetDevice(dstDev);
                err = cudaMemcpy2DAsync(dst, dpitch, hostBuffer[dstDevIndex], dpitch, w, h, cudaMemcpyHostToDevice, cudaStreams[dstDev]);
                err = cudaStreamSynchronize(cudaStreams[dstDev]);
    ...
    if(commType.transmissionType == asyncComm){
        ...
        for(i = 0; i<nGPUs; i++){
            ...
            cudaStreamSynchronize(cudaStreams[idGPUs[i]]);
    ...
    if(hostBuffer){
        ...
        for(size_t i = 0; i<nReconfGPUs; i++){
...
void reconfShrink(DTI_t *DTI){
    ...
    for(i = 0; i<nReconfGPUs; i++){
        ...
        // allocate hostBuffer
        hostBuffer[i] = (float*)malloc(DTI->nPerGPU[i] * sizeof(float));
    ...
    for(i = 0; i<nGPUs; i++){
        ...
        if(canAccess){
            ...
            if(commType.transferSteps != stridedComm){
                ...
                for(size_t nP = 0; nP < nPartitions; nP++){
                    ...
                    if (canAccess){
                        ...
                        if(commType.transmissionType == asyncComm){
                            ...
                            err = cudaMemcpyPeerAsync(dst, dstDev, src, srcDev, nBytes, cudaStreams[srcDev]);
                        ...
                        if(commType.transmissionType == asyncComm){
                            ...
                            err = cudaMemcpyAsync(dst, src, nBytes, cudaMemcpyDeviceToDevice, cudaStreams[srcDev]);
                            //cudaMemcpyPeerAsync(dst, dstDev, src, srcDev, DTI->nPerGPU[i] * (size_t)sizeof(float), cudaStreams[i]);
                ...
                if(commType.transmissionType == asyncComm){
                    err = cudaMemcpy2DAsync(dst, dpitch, src, spitch, w, h, cudaMemcpyDeviceToDevice, cudaStreams[srcDev]);
        ...
        // move data firt to the CPU, and then to the GPU
            ...
            // move data
            cudaSetDevice(srcDev);
            cudaMemcpy2DAsync(hostBuffer[dstDevIndex] + offset * prev_nPartitionGPU, dpitch, src, spitch, w, h, cudaMemcpyDeviceToHost, cudaStreams[srcDev]);
            err = cudaStreamSynchronize(cudaStreams[srcDev]);
            
            cudaSetDevice(dstDev);
            err = cudaMemcpy2DAsync(dst, dpitch, hostBuffer[dstDevIndex] + offset * prev_nPartitionGPU, dpitch, w, h, cudaMemcpyHostToDevice, cudaStreams[dstDev]);
            err = cudaStreamSynchronize(cudaStreams[dstDev]);
        }
    }        
...
void reconfN2N(DTI_t *DTI){
    ...
    // allocate memory for intermediate CPU buffers
    float **hostBuffer = (float**)calloc(nGPUs, sizeof(float*));
    ...
    for(i = 0; i<nGPUs; i++){
        ...
        hostBuffer[i] = (float*)malloc(DTI->nPerGPU[dstDevIndex] * sizeof(float));
        ...
        if(canAccess) {
            ...
            if (canAccess){
                ...
                if(commType.transmissionType == asyncComm){
                    ...
                    cudaMemcpyAsync(dst, src, nBytes, cudaMemcpyDeviceToDevice, cudaStreams[srcDev]);
                    //cudaMemcpyPeerAsync(dst, dstDev, src, srcDev, DTI->nPerGPU[i] * (size_t)sizeof(float), cudaStreams[i]);
                }
                else{
        ...
        // GPU - CPU - GPU
            ...
            cudaSetDevice(srcDev);
            cudaMemcpyAsync(hostBuffer[i], src, nBytes, cudaMemcpyDeviceToHost, cudaStreams[srcDev]);
            cudaStreamSynchronize(cudaStreams[srcDev]);

            cudaSetDevice(dstDev);
            cudaMemcpyAsync(dst, hostBuffer[i], nBytes, cudaMemcpyHostToDevice, cudaStreams[dstDev]);
            cudaStreamSynchronize(cudaStreams[dstDev]);
        }
    }
