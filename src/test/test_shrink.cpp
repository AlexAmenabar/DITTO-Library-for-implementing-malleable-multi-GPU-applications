...
// app functions
void shrinkTest1(int argc, void* argv[]){
...
// app functions
void shrinkTest2(int argc, void* argv[]){
...
int reconfShrinkTest1(schInfo_t *schInfo){
        ...
        for(size_t dstgpus = 1; dstgpus <= gpus / 2; dstgpus*=2){
            ...
            for(size_t nP = 0; nP<nPartitions; nP++){
                ...
                for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
                    ...
                    jobLauncher->launchFunc = &shrinkTest1;
...
int reconfShrinkTest2(schInfo_t *schInfo){
...
int reconfShrinkTest3(schInfo_t *schInfo){
    ...
    for(size_t nP = 0; nP<nPartitions; nP++){
        ...
        for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
            ...
            jobLauncher->argc = 6;
            jobLauncher->argv = jargs; // 2 extra arguments: whether the job is malleable and the job control (latter set)
            jobLauncher->launchFunc = &shrinkTest2;
            ...
            reconfJobResources->idGPUs = (size_t*)calloc(reconfJobResources->nGPUs, sizeof(size_t));
            reconfJobResources->idGPUs[0] = 1;
            reconfJobResources->idGPUs[1] = 0;
...
// topology-aware
int reconfShrinkTest4(schInfo_t *schInfo){
    ...
    for(size_t nP = 0; nP<nPartitions; nP++){
        ...
        for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
            ...
            reconfJobResources->nGPUs = 2;
            reconfJobResources->idGPUs = (size_t*)calloc(reconfJobResources->nGPUs, sizeof(size_t));
            reconfJobResources->idGPUs[0] = 2;
            reconfJobResources->idGPUs[1] = 3;
...
int main(int argc, char* argv[]){
    ...
    printf("\n\n\n[TEST 1]\n");
    fflush(stdout);
    reconfShrinkTest1(schInfo);

    printf("\n\n\n[TEST 2]\n");
    fflush(stdout);
    reconfShrinkTest2(schInfo);

    printf("\n\n\n[TEST 3]\n");
    fflush(stdout);
    reconfShrinkTest3(schInfo);

    printf("\n\n\n[TEST 4]\n");
    fflush(stdout);
    reconfShrinkTest4(schInfo);
}