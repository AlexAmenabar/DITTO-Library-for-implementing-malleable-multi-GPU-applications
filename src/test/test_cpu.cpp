...
// app functions
void cpuTest1(int argc, void* argv[]){
...
// app functions
void cpuTest2(int argc, void* argv[]){
...
// From SRC GPUs to the same DST GPUs
int reconfCPUTest1(schInfo_t *schInfo){
    ...
    for(size_t gpus = 1; gpus<= nGPUs; gpus *= 2){
        ...
        for(size_t nP = 0; nP<nPartitions; nP++){
            ...
            for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
                ...
                jobLauncher->launchFunc = &cpuTest1;
...
// From SRC GPUs to different DST GPUs
int reconfCPUTest2(schInfo_t *schInfo){
...
// Randomly ordered
int reconfCPUTest3(schInfo_t *schInfo){
    ...
    for(size_t nP = 0; nP<nPartitions; nP++){
        ...
        for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
            ...
            jobLauncher->argc = 6;
            jobLauncher->argv = jargs; // 2 extra arguments: whether the job is malleable and the job control (latter set)
            jobLauncher->launchFunc = &cpuTest2;
...
int main(int argc, char* argv[]){
    ...
    printf("\n\n\n\n\n[TEST 1]\n");
    fflush(stdout);
    reconfCPUTest1(schInfo);

    printf("\n\n\n\n\n[TEST 2]\n");
    fflush(stdout);
    reconfCPUTest2(schInfo);

    printf("\n\n\n\n\n[TEST 3]\n");
    fflush(stdout);
    reconfCPUTest3(schInfo);
}