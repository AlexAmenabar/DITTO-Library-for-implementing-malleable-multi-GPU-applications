...
// app functions
void keepTest1(int argc, void* argv[]){
...
// app functions
void keepTest2(int argc, void* argv[]){
...
int reconfKeepTest1(schInfo_t *schInfo){
    ...
    for(size_t gpus = 1; gpus<= nGPUs; gpus *= 2){
        ...
        for(size_t nP = 0; nP<nPartitions; nP++){
            ...
            for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
                ...
                jobLauncher->launchFunc = &keepTest1;
...
int reconfKeepTest2(schInfo_t *schInfo){
...
int reconfKeepTest3(schInfo_t *schInfo){
    ...
    for(size_t nP = 0; nP<nPartitions; nP++){
        ...
        for(size_t commMode = 0; commMode < nCommunicationModes; commMode++){
            ...
            jobLauncher->argc = 6;
            jobLauncher->argv = jargs; // 2 extra arguments: whether the job is malleable and the job control (latter set)
            jobLauncher->launchFunc = &keepTest2;
...
int main(int argc, char* argv[]){
    ...
    printf("\n\n\n[TEST 1]\n");
    fflush(stdout);
    reconfKeepTest1(schInfo);

    printf("\n\n\n[TEST 2]\n");
    fflush(stdout);
    reconfKeepTest2(schInfo);

    printf("\n\n\n[TEST 3]\n");
    fflush(stdout);
    reconfKeepTest3(schInfo);
}